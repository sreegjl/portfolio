...
(function () {
  ...
  // Note folders that have a nested flashcards/ page (value = deck key, unused
  // for the link itself since flashcards/ and quiz/ sit inside the note folder)
  var resourceMap = {
    'kinematics':      { fc: 'kinematics' },
    'force-dynamics':  { fc: 'dynamics' },
    'forces':          { fc: 'forces' },
    'circular-motion': { fc: 'circular-motion' },
  ...
  fetch('/notes-manifest.json')
    ...
    .then(function (manifest) {
      var allNotes = (manifest.htmlNotes || []).filter(function (n) { return n.path.indexOf('/quiz/') === -1; });
      ...
      if (idx !== -1) {
        ...
        var folder = path.replace(base, '').replace(/\/$/, '').split('/').pop();
        res = resourceMap[folder] || null;
      } else {
        // Practice page (<note>/quiz/) — prev = its note, next = note after it
        if (!/\/quiz\/$/.test(path)) return;
        var notePath = path.replace(/quiz\/$/, '');
        var noteIdx = -1;
        allNotes.forEach(function (n, i) {
          if (base + n.path.replace(/\/index\.html$/, '') + '/' === notePath) noteIdx = i;
        });
        if (noteIdx === -1) return;
        isPractice = true;
